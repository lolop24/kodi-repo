...
MAX_WORKERS = 4
ITEMS_PER_PAGE = 15
RANKING_CACHE_TTL = 12 * 60 * 60
DETAIL_CACHE_TTL = 20 * 60 * 60  # 20 hours
RANKING_CACHE_VER = 8


def log(msg, level=xbmc.LOGINFO):
    xbmc.log('[CSFD Lite] %s' % msg, level)
...
def _save_json(filename, data):
    ...
    tmp_path = None
    try:
        fd, tmp_path = tempfile.mkstemp(prefix='%s.' % filename, suffix='.tmp', dir=directory)
...
def fetch_details_for_page(items, silent=False):
    ...
    if not silent:
        ...
        progress.create(_L(30101), _L(30112) % (0, len(to_fetch)))
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
            ...
            for f in as_completed(futs):
                ...
                if progress:
                    ...
                    progress.update(pct, _L(30112) % (done, len(to_fetch)))
    finally:
        if progress:
            progress.close()
...
def fetch_single_title_data(csfd_url):
    """Fetch original/English title data for one item on demand."""
    cache = _load_json('detail_cache.json')
    now = time.time()
    key = _md5(csfd_url)
    ...
    if entry and (now - entry.get('ts', 0)) < DETAIL_CACHE_TTL:
        if entry.get('original_title') or entry.get('english_title'):
            return {
    ...
    entry.update({
        ...
        'english_title': det.get('english_title', ''),
        'plot': det.get('plot', entry.get('plot', '')),
        'reviews': det.get('reviews', entry.get('reviews', [])),
        'ts': now,
    })
    cache[key] = entry
    _save_json('detail_cache.json', cache)
